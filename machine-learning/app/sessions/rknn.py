...
from rknn.rknn import Ann  # Import from rknn module
...
class RKNNSession:
    """
    Wrapper for RKNN to be a drop-in replacement for ONNX session.
    """

    def __init__(self, model_path: Path, cache_dir: Path = settings.cache_folder) -> None:
        self.model_path = model_path
        self.cache_dir = cache_dir
        self.ann = Ann(tuning_level=settings.ann_tuning_level, tuning_file=(cache_dir / "npu-tuning.rknn").as_posix())

        log.info("Loading RKNN model %s ...", model_path)
        self.model = self.ann.load(
            model_path.as_posix(),
            cached_network_path=model_path.with_suffix(".rknncache").as_posix(),
    ...
    def get_inputs(self) -> list[SessionNode]:
        ...
        return [RKNNNode(None, s) for s in shapes]
